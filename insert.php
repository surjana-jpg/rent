    ...
    if (!file_exists($targetFilePath)) {
        if(in_array($fileType, $allowTypes)){
            ...
            if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
                ...
                $insert = $conn->query("INSERT into rooms (address, price, contact, details, username,file_name, uploaded_on) VALUES ('$address', '$price','$contact','$details', '$username','".$fileName."', NOW())");
...
// Close connection
mysqli_close($link);
header("refresh:0; url = room1.php");
?>
